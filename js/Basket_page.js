var basket_page = null;
window.addEvent('domready', function() {
	basket_page = new Basket_page();
});


var Basket_page = new Class({
	initialize : function() {
		this.my_cart = $('open_sub_cart');
		this.products = $$('#basket_table tbody tr.product');
		this.basket_items_list = $$('#basket_table tbody')[0];
		
		$$('#basket_table tbody tr.product').each(function(product, index){
			new BasketItem_page(product);
		}.bind(this));
	...
	updateBasketPrice: function(){
		$$('.content_basket #order_subtotal #order_subtotal_value').getElement('span').set('text', this.getBasketPrice());
	},
	...
	getBasketItemProductHTML: function(product_datas){
		...
		html += '	</td>';
		html += '	<td>';
		html += '		<div class="time_remaining"><span>'+product_datas.basket_item_time_remaining+'</span> min.</div>';
		html += '	</td>';
		html += '	<td>';
	...
	setNbProductsInBasket: function(){
		...
		if(this.my_cart.getElement('#in_cart'))
		if(nb_products == 0){
...
var BasketItem_page = new Class({
	initialize : function(product) {
		...
		this.product.getElements('a.remove')[0].addEvent('click', function(e){
			e.stop();
			this.disableCounter();
			basket_page.deleteBasketItem(this.product);
		}.bind(this));

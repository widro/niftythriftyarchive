...
var Basket = new Class({
	initialize : function() {
		...
		this.add_item.setStyles({
			'display' : 'block',
			'opacity' : 0
		});
		
		this.add_item.store('fx', new Fx.Morph(this.add_item, {
		    link: 'cancel',
		    duration: 500
		}));
		...
		$$('div#sub_cart.sub table tbody tr.product').each(
	...
	getBasketPrice : function() {
		...
		this.products.each(function(product, index) {
			basket_price += product.getElements('div.price_final span')[0]
					.get('text') * 1;
		});
	...
	updateBasketPrice : function() {
		$('order_subtotal_value').getElement('span').set('text',
				this.getBasketPrice());
		...
		new Request.JSON({
			...
			onComplete : function() {
				...
				if ($$('#sub_cart tr.product').length == 0) {
					$$('#sub_cart #no_products')[0].setStyles({
						'display' : 'block'
					});
				}
				scrollbarCart.update();
	...
	showAddItem: function(){
		this.add_item.retrieve('fx').start({
            'opacity': 1
        }).chain(function(){
        	this.add_item.retrieve('fx').start.delay(4000, this.add_item.retrieve('fx'), {
                'opacity': 0
            });
        }.bind(this));
	},
		...
		new Request.JSON({
			...
			onComplete : function(xhr) {
				if (!xhr.error.length) {
					...
					$('ajax_result').removeClass('error').addClass(
							'add_success').set('text', xhr.success);
					
					this.showAddItem();
				} else {
					$('ajax_result').removeClass('success').addClass(
							'add_error').set('text', xhr.error);
				}
				scrollbarCart.update();
	...
	getBasketItemProductHTML : function(product_datas) {
		var html = '';
		html += '	<td><a href="' + product_datas.product_url
				+ '" title=""><img class="cart_pic" src="'
				+ product_datas.product_visual1 + '" /></a></td>';
		html += '	<td>';
		html += '		<div class="description_name">' + product_datas.product_name
				+ '</div>';
		html += '		<div class="description_size">'
				+ product_datas.product_category_size_value + '</div>';
		html += '	</td>';
		html += '	<td>';
		html += '		<div class="delivery">' + product_datas.basket_item_delivery
				+ '</div>';
		html += '	</td>';
		html += '	<td>';
		html += '		<div class="time_remaining active"><span>'
				+ product_datas.basket_item_time_remaining
				+ '</span> min.</div>';
		html += '	</td>';
		html += '	<td>';
		html += '		<div class="price_final">$<span>'
				+ product_datas.basket_item_price + '</span></div>';

		if (product_datas.basket_item_discount != 0) {
			html += '			<div class="price">$<span>'
					+ product_datas.basket_item_final_price + '</span></div>';
		}

		html += '	</td>';
		html += '	<td>';
		html += '		<a href="' + product_datas.basket_item_delete_from_basket
				+ '" class="remove"></a>';
		html += '	</td>';
	...
	setNbProductsInBasket : function() {
		...
		//$('items_in_cart').set('text', this.products.length);
		this.centerMyCart();
	},
...
var BasketItem = new Class(
		{
			initialize : function(product) {
				...
				this.time_remaining = this.product
						.getElements('div.time_remaining')[0];

				if (this.time_remaining.hasClass('active')) {
					var tmp = this.time_remaining.getElement('span')
							.get('text').split(':');
				...
				/*
				if(this.product.getElements('span.remove')){
				this.product.getElements('span.remove')[0].addEvent('click',
						function(e) {
							e.stop();
							this.disableCounter();
			...
			setTimeRemaining : function() {
				this.time_remaining.getElement('span').set(
						'text',
						this.formatNumber(this.time_remaining_min) + ':'
								+ this.formatNumber(this.time_remaining_sec));
			},
